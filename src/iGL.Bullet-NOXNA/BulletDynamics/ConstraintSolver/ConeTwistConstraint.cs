...
namespace BulletXNA.BulletDynamics
{
	public class ConeTwistConstraint : TypedConstraint
	{
		...
		public ConeTwistConstraint(RigidBody rbA, RigidBody rbB, ref Matrix rbAFrame, ref Matrix rbBFrame) :
			base(TypedConstraintType.ConeTwist, rbA, rbB)
		{
			m_angularOnly = false;
			m_rbAFrame = rbAFrame;
			m_rbBFrame = rbBFrame;
		...
		public ConeTwistConstraint(RigidBody rbA, ref Matrix rbAFrame)
			: base(TypedConstraintType.ConeTwist, rbA)
		{
			m_rbAFrame = rbAFrame;
			m_rbBFrame = rbAFrame;
			m_angularOnly = false;
        ...
        public static float ComputeAngularImpulseDenominator(ref Vector3 axis, ref IndexedBasisMatrix invInertiaWorld)
		{
            //Vector3 vec = MathUtil.TransposeTransformNormal(axis, invInertiaWorld);
            Vector3 vec = axis* invInertiaWorld;
        ...
        public void GetInfo2NonVirtual(ConstraintInfo2 info, Matrix transA, Matrix transB, IndexedBasisMatrix invInertiaWorldA, IndexedBasisMatrix invInertiaWorldB)
		{
			...
			Vector3 a1 = transA._basis * m_rbAFrame.Translation;
			{
				Vector3 a1neg = -a1;
            ...
            Vector3 a2 = transB._basis * m_rbBFrame.Translation;
			{
			...
			for (int j = 0; j < 3; j++)
			{
				info.m_solverConstraints[j].m_rhs = k * (a2[j] + transB.Translation[j] - a1[j] - transA.Translation[j]);
			...
			if (m_solveSwingLimit)
			{
				if ((m_swingSpan1 < m_fixThresh) && (m_swingSpan2 < m_fixThresh))
				{
					...
					Vector3 p = trA._basis.GetColumn(1);
					Vector3 q = trA._basis.GetColumn(2);
		...
		public void CalcAngleInfo()
		{
			...
			if (m_twistSpan >= 0f)
			{
                Vector3 b2Axis2a = GetRigidBodyB().GetCenterOfMassTransform()._basis * m_rbBFrame._basis.GetColumn(1); 
        ...
        public void CalcAngleInfo2(Matrix transA, Matrix transB, IndexedBasisMatrix invInertiaWorldA, IndexedBasisMatrix invInertiaWorldB)
		{
			CalcAngleInfo2(ref transA, ref transB, ref invInertiaWorldA, ref invInertiaWorldB);
		}

        public void CalcAngleInfo2(ref Matrix transA, ref Matrix transB, ref IndexedBasisMatrix invInertiaWorldA, ref IndexedBasisMatrix invInertiaWorldB)
		{
			m_swingCorrection = 0;
			m_twistLimitSign = 0;
			m_solveTwistLimit = false;
...

			{
				...
				else
				{
                    ...
                    Vector3 ivA = transA._basis * m_rbAFrame._basis.GetColumn(0);
                    Vector3 jvA = transA._basis * m_rbAFrame._basis.GetColumn(1);
                    Vector3 kvA = transA._basis * m_rbAFrame._basis.GetColumn(2);
                    Vector3 ivB = transB._basis * m_rbBFrame._basis.GetColumn(0);
		...
		public void SetMotorTarget(ref Quaternion q)
		{
			...
			Matrix trABCur = trBCur.Inverse() * trACur;
			Quaternion qABCur = trABCur.GetRotation();
            Matrix trConstraintCur = (trBCur * m_rbBFrame).Inverse() * (trACur * m_rbAFrame);
        ...
        public override void BuildJacobian()
        {
            if (m_useSolveConstraintObsolete)
            {
                ...
                m_accMotorImpulse = Vector3.Zero;

                if (!m_angularOnly)
                {
                    Vector3 pivotAInW = m_rbA.GetCenterOfMassTransform() * m_rbAFrame.Translation;
                    Vector3 pivotBInW = m_rbB.GetCenterOfMassTransform() * m_rbBFrame.Translation;
        ...
        public void solveConstraintObsolete(RigidBody bodyA, RigidBody bodyB, float timeStep)
{
	if (m_useSolveConstraintObsolete)
	{
		...
		if (!m_angularOnly)
		{
			Vector3 rel_pos1 = pivotAInW - m_rbA.GetCenterOfMassPosition(); 
			Vector3 rel_pos2 = pivotBInW - m_rbB.GetCenterOfMassPosition();
			...
			for (int i=0;i<3;i++)
			{		
				Vector3 normal = m_jac[i].m_linearJointAxis;
				...
				Vector3 ftorqueAxis1 = rel_pos1.Cross(ref normal);
				Vector3 ftorqueAxis2 = rel_pos2.Cross(ref normal);
		...
		if (m_bMotorEnabled)
		{
			...
			Matrix trACur = m_rbA.GetCenterOfMassTransform();
			Matrix trBCur = m_rbB.GetCenterOfMassTransform();
			Vector3 omegaA = Vector3.Zero; bodyA.InternalGetAngularVelocity(ref omegaA);
			...
			// compute desired transforms in world
			Matrix trPose = Matrix.CreateFromQuaternion(m_qTarget);
			Matrix trABDes = m_rbBFrame * trPose * m_rbAFrame.Inverse();
			Matrix trADes = trBPred * trABDes;
			Matrix trBDes = trAPred * trABDes.Inverse();

			// compute desired omegas in world
			Vector3 omegaADes, omegaBDes;
			...
			Vector3 dOmegaA = omegaADes - omegaA;
			Vector3 dOmegaB = omegaBDes - omegaB;

			// compute weighted avg axis of dOmega (weighting based on inertias)
            Vector3 axisA = Vector3.Zero, axisB = Vector3.Zero;
			...
			Vector3 avgAxis = kAxisAInv * axisA + kAxisBInv * axisB;
			...
			if (bDoTorque && avgAxis.LengthSquared() > MathUtil.SIMD_EPSILON)
			{
				...
				Vector3 impulse = (kAxisAInv * dOmegaA - kAxisBInv * dOmegaB) /
				...
				if (m_maxMotorImpulse >= 0)
				{
					...
					Vector3 newUnclampedAccImpulse = m_accMotorImpulse + impulse;
				...
				Vector3 impulseAxis =  impulse / impulseMag;

				bodyA.InternalApplyImpulse(new Vector3(0,0,0), m_rbA.GetInvInertiaTensorWorld()*impulseAxis, impulseMag,null);
				bodyB.InternalApplyImpulse(new Vector3(0,0,0), m_rbB.GetInvInertiaTensorWorld()*impulseAxis, -impulseMag,null);

			}
		}
		else if (m_damping > MathUtil.SIMD_EPSILON) // no motor: do a little damping
		{
			Vector3 angVelA = Vector3.Zero; bodyA.InternalGetAngularVelocity(ref angVelA);
			Vector3 angVelB= Vector3.Zero; bodyB.InternalGetAngularVelocity(ref angVelB);
			Vector3 relVel = angVelB - angVelA;
			if (relVel.LengthSquared() > MathUtil.SIMD_EPSILON)
			{
				Vector3 relVelAxis = relVel.Normalized();
				...
				Vector3 impulse = m_damping * m_kDamping * relVel;
		...
		// joint limits
		{
			...
			if (m_solveSwingLimit)
			{
				...
				{
					Vector3 impulseTwistCouple = impulse.Dot(ref m_twistAxisA) * m_twistAxisA;
					Vector3 impulseNoTwistCouple = impulse - impulseTwistCouple;
					impulse = impulseNoTwistCouple;
				}

				impulseMag = impulse.Length();
				Vector3 noTwistSwingAxis = impulse / impulseMag;

				bodyA.InternalApplyImpulse(new Vector3(0,0,0), m_rbA.GetInvInertiaTensorWorld()*noTwistSwingAxis, impulseMag,null);
				bodyB.InternalApplyImpulse(new Vector3(0,0,0), m_rbB.GetInvInertiaTensorWorld()*noTwistSwingAxis, -impulseMag,null);
			}
			...
			if (m_solveTwistLimit)
			{
				...
				bodyA.InternalApplyImpulse(new Vector3(0,0,0), m_rbA.GetInvInertiaTensorWorld()*m_twistAxis,impulseMag,null);
				bodyB.InternalApplyImpulse(new Vector3(0,0,0), m_rbB.GetInvInertiaTensorWorld()*m_twistAxis,-impulseMag,null);
			}		
		}
	}
