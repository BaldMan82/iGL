...
{	
	...
	mediump vec4 furColor = texture2D(s_normalTexture, v_uv);

	lowp vec3 normal = v_normal;
	lowp vec3 lightVector = v_lightVector;	
	
	calcLightning(color, textureColor*furColor, normal, lightVector);

	if (u_hasNormalTexture > 0 )
	{
		color = vec4(textureColor.r, textureColor.g, textureColor.b, furColor.a); 
		calcLightning(color, color, normal, lightVector);		
	}
	else 
	{
